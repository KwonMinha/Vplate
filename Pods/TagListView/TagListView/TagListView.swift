...
open class TagListView: UIView {
    ...
    private func createNewTagViewWithImage(_ title: String, _ image: UIImage) -> TagView {
        ...
        tagView.addTarget(self, action: #selector(tagPressed(_:)), for: .touchUpInside)
        tagView.removeButton.addTarget(self, action: #selector(removeButtonPressed(_:)), for: .touchUpInside)
        tagView.removeButton.imageEdgeInsets = UIEdgeInsets(top: 5, left: 10, bottom: 5, right: 4 )
        tagView.removeButton.setImage(image, for: .normal)
        // On long press, deselect all tags except this one
        tagView.onLongPress = { [unowned self] this in
            for tag in self.tagViews {
                tag.isSelected = (tag == this)
    ...
    open func addTagsWithImage(_ datas: [UIImage : String]) -> [TagView] {
        var tagViews: [TagView] = []
        for data in datas {
            tagViews.append(createNewTagViewWithImage(data.value, data.key))
        }
        return addTagViews(tagViews)
    }
